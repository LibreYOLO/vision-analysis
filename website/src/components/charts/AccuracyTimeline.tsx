...
export function AccuracyTimeline() {
  ...
  useEffect(() => {
    ...
    // Lines stop at last release - no extension to the right
    const companies = Object.keys(COMPANY_COLORS).filter((c) =>
      COMPANY_TIMELINE.some((d) => d.company === c)
    );
    ...
    const plot = Plot.plot({
      ...
      marks: [
        ...
        // Step lines per company - stops at their last data point
        ...
        // Square dots at release points (like AA)
  ...
  return (
    <div>
      <div className="pt-2">
        {/* Legend on top like AA */}
        <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3 px-1">
          {Object.entries(COMPANY_COLORS).map(([org, color]) => (
